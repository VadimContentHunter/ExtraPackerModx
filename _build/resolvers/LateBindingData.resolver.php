...
function bindData(
    ...
    $dependentUseFieldName      = $referenceData['dependentUseFieldName'] ?? '';
    ...
    $objResource = $modx->getObject($className, [
        $dependentSearchFieldName => $dependentKey
    ]);
...
if (
    ...
    if (isset($options['LateBindingData'])) {
        foreach ($options['LateBindingData'] as $className => $objects) {
            ...
            foreach ($objects as $dependentKey => $data) {
                bindData($modx, $className, $dependentKey, $data);
            }
        }
    } else {
